...
<article class="space-y-8">
	<header class="space-y-4 border-b border-gray-200 pb-8 dark:border-gray-800">
		<div class="flex items-center justify-between">
			<a
				href={resolve('/')}
				class="inline-flex items-center text-sm text-gray-600 transition-colors hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-400"
			>
		...
		<div class="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
			{#if data.metadata.author}
				<span
					>{m.by()}
					<span class="text-blue-500 dark:text-blue-400">{data.metadata.author}</span></span
				>
				<span>•</span>
			{/if}
			<time>
